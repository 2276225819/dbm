...
echo $conn->session(Post::class)->delete(true)."\n";

echo "#sql[Post::class]->insert\n";
$conn->session(Post::class)->insert([
...
$conn->session(User::class)->load(2)->ref(Post::class)->insert([ 
	'text'=>'insertUser' ,'post_type_id'=>1
]); 

echo "#sql[Post::class][0][PostType::class]->insert\n";
$conn->session(Post::class)->get(0)->ref(PostType::class)->insert([
	'name'=>'type new'
]);
echo "---------------\n";
...
$conn->session(Post::class)->load(7)->ref(PostType::class)->update([
	'name'=>'type5'
]); 
$conn[Post::class]->map(function(Post $value){
	echo json_encode($value); echo "\n"; 
