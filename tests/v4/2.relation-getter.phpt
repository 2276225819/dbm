...
$b=$conn->model('zz_user','Id')->ref('zz_post','Id',['user_id'=>'Id'])->val('text');  
print_r([$a,$b]); 

//<!--SELECT * FROM `zz_user`  WHERE (`Id`=?)  ;1-->
//<!--SELECT * FROM `zz_post`  WHERE `user_id`=?  ;1--> 
$a=$conn[User::class](1)[Post::class]['text']; 
$b=$conn->model('zz_user','Id')->load(1)->ref('zz_post','Id',['user_id'=>'Id'])->val('text'); 
print_r([$a,$b]);

//<!--SELECT * FROM `zz_user`    LIMIT 1 OFFSET 1 ;-->
//<!--SELECT * FROM `zz_post`  WHERE `user_id`=?  ;2-->
$a=$conn[User::class][1][Post::class]['text']; 
$b=$conn->model('zz_user','Id')->get(1)->ref('zz_post','Id',['user_id'=>'Id'])->val('text'); 
print_r([$a,$b]); 

$a=$conn->model('zz_user','Id')->find(2)
